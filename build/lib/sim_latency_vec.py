...
def build_env(args) -> AsyncVectorEnv:
    # Choose a reduced action set when requested.
    # Pong only needs {NOOP, FIRE, RIGHT, LEFT}; otherwise default to UDLR.
    if args.reduce_action_set:
        if args.rom.lower() == "pong":
            action_map = np.asarray([0, 1, 3, 4], dtype=np.int64)
        else:
            action_map = np.asarray([2, 5, 4, 3], dtype=np.int64)  # UDLR
    else:
        action_map = None
    ...
    def make_env(rank: int):
        def thunk():
            ...
            # For SwiftTD-style features, we need raw RGB frames (210x160x3)
            # Our AtariFeatureExtractor handles the resize to 105x80 and binning
            # Only use AtariPreprocessing if grayscale is explicitly requested
            ...
            # Otherwise return raw RGB - SwiftTD features need this!
            return env
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--use_grayscale", action="store_true", help="Use grayscale preprocessing (default: raw RGB for SwiftTD).")
            ...
            if args.log_interval and global_step % args.log_interval < args.num_envs:
                ...
                if run is not None:
                    ...
                    if core is not None:
                        ...
                        # Replay buffer metrics (only for agents with replay)
                        replay = getattr(core, "replay", None)
                        if replay is not None:
                            metrics["train/beta"] = getattr(replay, "beta", 0.0)
                            if getattr(replay, "max_priority", None) is not None:
                                metrics["replay/max_priority"] = replay.max_priority
                            if hasattr(replay, "size") and hasattr(replay, "capacity"):
                                metrics["replay/fraction_filled"] = replay.size / float(replay.capacity)
                        ...
                        network = getattr(core, "network", None)
