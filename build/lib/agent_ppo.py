...
        self.n_stack = 16  # Number of frames to stack (matches sim training)
...
        logger.info(f"agent_ppo: Wandb logging enabled: {self.use_wandb}")
...
        logger.info(f"agent_ppo: Policy outputs {self.policy_num_actions} actions (indices into action_set)")
...
        logger.info(f"agent_ppo: Environment num actions = {num_actions}")
...
            gamma=self.gamma,
            gae_lambda=self.gae_lambda,
            n_envs=1  # We're using a single environment
        )
...
        # observation_rgb8 is (210, 160, 3) from physical env
...
        if self.last_obs is not None and not self.is_training:
...
                reward=np.array([self.last_reward]),
...
            if self.use_wandb and self.frames_since_train % 100 == 0:
                wandb.log({
...
        logger.info(f"agent_ppo: Training step {self.training_step} started")
