...
from swiftsarsa import SwiftSarsa, SwiftSarsaBinaryFeatures
...
def main():
    ...
    parser.add_argument("--meta_step_size", type=float, default=1e-3,
                        help="Meta step size (theta) for Swift-Sarsa")
    parser.add_argument("--eta", type=float, default=5.0,
                        help="Eta clamp for Swift-Sarsa step sizes")
    ...
    parser.add_argument("--video_dir", type=str, default=None,
                        help="If set, save episode videos (mp4) to this folder via ffmpeg")
    parser.add_argument("--log_actions_every", type=int, default=0,
                        help="If >0, log chosen policy_action->ale_action every N steps")
    parser.add_argument("--log_progress_every", type=int, default=0,
                        help="If >0, log training stats every N global steps")
    parser.add_argument("--softmax_only", action="store_true",
                        help="Ignore epsilon-greedy; use softmax-only exploration")
    parser.add_argument("--save_betas_every", type=int, default=0,
                        help="If >0, save beta heatmaps every N steps")
    parser.add_argument("--betas_dir", type=str, default=None,
                        help="Directory to save beta heatmaps (requires save_betas_every>0)")
    parser.add_argument("--log_betas_every", type=int, default=0,
                        help="If >0, print beta/step-size stats every N steps")
    ...
    # Optional video recording (ffmpeg via imageio)
    writer_factory = None
    if args.video_dir:
        ...
        try:
            ...
            def make_writer(path: str, fps: int = 15):
                # macro_block_size=1 prevents imageio-ffmpeg from resizing frames (avoids warnings)
    ...
    betas_dir = None
    if args.save_betas_every > 0 and args.betas_dir:
    ...
    if args.reduce_action_set and args.rom.lower() == "pong":
        # Prefer RIGHT/LEFT (horizontal) controls per minimal Pong action space.
        # In full action space the indices are typically: 0=NOOP, 1=FIRE, 3=RIGHT, 4=LEFT.
        noop_idx, fire_idx, right_idx, left_idx = 0, 1, None, None
        if meanings:
            for i, m in enumerate(meanings):
                mu = m.upper()
                if mu == "RIGHT" and right_idx is None:
                    right_idx = i
                if mu == "LEFT" and left_idx is None:
                    left_idx = i
        # Fallbacks: minimal 6-action layout (0 NOOP, 1 FIRE, 2 RIGHT, 3 LEFT)
        right_idx = 2 if right_idx is None else right_idx
        left_idx = 3 if left_idx is None else left_idx

        # Policy actions: 0=NOOP, 1=FIRE, 2=RIGHT, 3=LEFT
        action_map = np.array([noop_idx, fire_idx, right_idx, left_idx], dtype=np.int64)
        num_actions = len(action_map)
        desc = []
        for idx in action_map:
            label = meanings[idx] if meanings and idx < len(meanings) else "n/a"
            desc.append(f"{idx}:{label}")
        print(f"Using reduced Pong action set (NOOP/FIRE/RIGHT/LEFT): {action_map.tolist()} -> {desc}")
    else:
        action_map = None
    ...
    exploration_eps = 0.0 if args.softmax_only else args.epsilon
    reward_clip = 1.0
    ...
    if args.use_memory:
        print("Memory features with binary learner are not supported yet; disabling use_memory.")
    memory = None
    free_mem_ids = deque()
    total_features = base_features

    # Swift-Sarsa learner (binary features)
    learner = SwiftSarsaBinaryFeatures(
        total_features,
        num_actions,
        0.99,      # lambda
        alpha,    # alpha
        args.meta_step_size,  # theta (meta step)
        args.eta,             # eta clamp
        0.999,    # decay
        1e-6,     # epsilon
        3.06e-7,  # eta_min (≈ e^-15)
    )
    print(f"learner alpha_init={learner.get_alpha_init()} (requested alpha={alpha})")
    ...
    def maybe_write_frame():
        ...
        if writer is None:
            ...
            writer = writer_factory(video_path, fps=int(60 / max(args.frame_skip, 1)))
        writer.append_data(frame_rgb)

    def maybe_save_betas(step_idx: int):
        """Save beta heatmap (per-feature step sizes) as a grayscale PNG."""
        if betas_dir is None or args.save_betas_every <= 0:
            return
        if step_idx % args.save_betas_every != 0:
            return
        try:
            import imageio
            betas = np.array(learner.get_feature_betas_max_over_actions(), dtype=np.float32)
            pix_count = extractor.height * extractor.width * extractor.num_channels * extractor.num_bins
            if betas.size < pix_count:
                print(f"Could not save betas: expected at least {pix_count}, got {betas.size}")
                return
            betas_pix = betas[:pix_count].reshape(
                extractor.height, extractor.width, extractor.num_channels, extractor.num_bins
            )
            # C++ returns |beta|; assume beta is negative log step-size and invert sign.
            heat = np.exp(-betas_pix)
            heat = heat.max(axis=(2, 3))
            span = float(heat.max() - heat.min())
            if span <= 1e-12:
                heat = np.exp(-np.abs(betas_pix)).max(axis=(2, 3))
                span = float(heat.max() - heat.min())
            heat = heat - heat.min()
            if span <= 1e-12:
                print("Could not save betas: heatmap is constant (no variation).")
                return
            heat = heat / (heat.max() + 1e-12)
            img = (heat * 255.0).clip(0, 255).astype(np.uint8)
            out_path = os.path.join(betas_dir, f"betas_step_{step_idx:07d}.png")
            imageio.imwrite(out_path, img)
            print(f"Saved beta heatmap to {out_path}")
        except Exception as e:
            print(f"Could not save betas: {e}")

    def maybe_log_betas(step_idx: int):
        """Log simple beta/step-size stats."""
        if args.log_betas_every <= 0 or (step_idx % args.log_betas_every != 0):
            return
        try:
            betas = np.array(learner.get_feature_betas_max_over_actions(), dtype=np.float32)
            # C++ getter returns |beta|; treat beta as negative log-alpha.
            step_sizes = np.exp(-betas)
            num_eligible = getattr(learner, "get_num_eligible", lambda: None)()
            eligible = int(num_eligible) if num_eligible is not None else None
            eligible_slots = total_features * num_actions
            eligible_frac = (eligible / eligible_slots) if (eligible_slots > 0 and eligible is not None) else None
            stats = {
                "beta_min": float(betas.min()),
                "beta_max": float(betas.max()),
                "beta_med": float(np.median(betas)),
                "beta_p99": float(np.percentile(betas, 99)),
                "alpha_min": float(step_sizes.min()),
                "alpha_max": float(step_sizes.max()),
                "alpha_med": float(np.median(step_sizes)),
                "alpha_p99": float(np.percentile(step_sizes, 99)),
                "eligible": eligible,
            ...
            for k, v in stats.items():
                if k == "eligible":
                    if v is None:
                        parts.append(f"{k}=n/a(rebuild swiftsarsa?)")
                    else:
                        parts.append(f"{k}={v}")
                elif k == "eligible_frac":
                    if v is None:
                    ...
                    parts.append(f"{k}={v:.3e}")
            print(f"betas step {step_idx}: " + "  ".join(parts))
        except Exception as e:
            print(f"Could not log betas: {e}")

    maybe_write_frame()

    base_indices_only = extractor.extract(frame)
    features = base_indices_only
    base_indices_prev = base_indices_only  # for next step's imprinting logic

    q_values = np.array(learner.get_action_values(features))
    if action_map is not None:
        action = 1  # FIRE once at episode start to serve the ball
    else:
        use_softmax = args.softmax_only or not args.pure_epsilon
        action = select_action(q_values, exploration_eps, temperature, use_softmax=use_softmax)

    # Prime learner with s0,a0 so v_old is initialized (no weight update: reward=0, gamma=0)
    if learning_enabled:
        learner.learn(features, 0.0, 0.0, action)

    # Store prev state/action for learn() call
    prev_features = features  # list of indices
    prev_action = action
    ...
    while global_step < args.decisions:
        ...
        if args.log_actions_every and (global_step % args.log_actions_every == 0):
            if action_map is not None:
                meaning_lookup = ["NOOP", "FIRE", "RIGHT", "LEFT"]
                meaning_str = meaning_lookup[prev_action] if 0 <= prev_action < len(meaning_lookup) else "n/a"
            elif meanings and 0 <= ale_action < len(meanings):
                meaning_str = meanings[ale_action]
            else:
                meaning_str = "n/a"
            print(f"act_log step {global_step}: policy_action={prev_action} -> ale_action={ale_action} ({meaning_str})")
        ...
        base_indices_next_only = extractor.extract(frame)
        features_next = base_indices_next_only
        ...
        use_softmax = args.softmax_only or not args.pure_epsilon
        action_next = select_action(q_values_next, exploration_eps, temperature, use_softmax=use_softmax)
        action_counts[action_next] += 1
        if learning_enabled:
            learner.learn(features_next, reward, g, action_next)
        maybe_save_betas(global_step)
        maybe_log_betas(global_step)
        ...
        if args.log_progress_every and (global_step % args.log_progress_every == 0):
            print(
                f"step {global_step}  "
