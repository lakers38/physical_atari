...
class VectorSwiftTDAgent:
    ...
    def __init__(
        ...
        self.critics = [self._build_critic() for _ in range(num_envs)]
        self.prev_values = np.zeros(num_envs, dtype=np.float32)
        self.prev_features = [None for _ in range(num_envs)]
    ...
    def start_episodes(self, obs_batch: np.ndarray):
        ...
        for i in range(self.num_envs):
            v = self.critics[i].step(feats_np[i].tolist(), 0.0)  # CRITICAL_LINE bootstrap V(s)
            self.prev_values[i] = v
            self.prev_features[i] = feats_np[i]
    ...
    def reset_done(self, dones: np.ndarray, obs_batch: np.ndarray):
        ...
        for i, done in enumerate(dones):
            if done:
                v = self.critics[i].step(feats_np[i].tolist(), 0.0)  # CRITICAL_LINE reset V(s) after terminal
                self.prev_values[i] = v
                self.prev_features[i] = feats_np[i]

    def _extract_features(self, obs_batch: np.ndarray) -> Tuple[torch.Tensor, np.ndarray]:
        assert obs_batch.shape == (self.num_envs, self.input_size, self.input_size, self.n_stack)
    ...
    def update(
        ...
        new_prev_values = np.zeros_like(self.prev_values)
        new_prev_features = [None for _ in range(self.num_envs)]

        for i in range(self.num_envs):
            V_current = float(self.prev_values[i])
            target_feats = np.zeros_like(next_feats_np[i]) if dones[i] else next_feats_np[i]  # CRITICAL_LINE absorbing-state handling
            V_next = self.critics[i].step(target_feats.tolist(), float(rewards[i]))  # CRITICAL_LINE SwiftTD update/query
            adv = float(rewards[i]) + (0.0 if dones[i] else self.gamma * V_next) - V_current  # CRITICAL_LINE TD error / advantage
            advantages[i] = adv

            if dones[i]:
                new_prev_values[i] = 0.0
                new_prev_features[i] = None
            else:
                new_prev_values[i] = V_next
                new_prev_features[i] = target_feats

        # Update caches
        self.prev_values = new_prev_values
        self.prev_features = new_prev_features

        # Actor loss over batch
    ...
    def load(self, path: str):
        ...
        self.prev_features = [None for _ in range(self.num_envs)]
