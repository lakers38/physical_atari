...
def test_bandit_prefers_rewarded_action(reward_prob, threshold, steps):
    ...
    for t in range(steps):
        ...
        prev_action = action
        print(f"o: {t}\ta: {action}\tr: {reward}\tbuffer_size: {agent.rollout_buffer.size()}\tfslt:{agent.frames_since_train}")
    ...
    assert probs[0][0] > .5, f"Distribution should skew towards action 0, but distribution was: {probs[0]}"
...
def test_bandit_reward_flip_adapts_policy():
    ...
    for t in range(steps):
        ...
        if t == flip_at:
            stacked_frames = np.zeros((1, 128, 128))
            obs_tensor = torch.as_tensor(stacked_frames).unsqueeze(0).to(agent.actor_model.device)
            pre_flip_probs = agent.actor_model.policy.get_distribution(obs_tensor).distribution.probs[0].detach().cpu().numpy()
