...
def main():
    ...
    parser.add_argument("--full_action_space", action="store_true",
                        help="Use full ALE action space (default is env's minimal action set).")
    ...
    if args.log_file:
        ...
        log_filename = (
            f"pong_{args.rom}_seed{args.seed}"
    ...
    # Reduced action set by default: [NOOP, RIGHT, LEFT]
    if args.full_action_space:
        action_map = None
        num_actions = env.action_space.n
        log("Action set: full ALE action space")
    else:
        action_map = [0, 3, 4]  # NOOP, RIGHT, LEFT in ALE Pong
        num_actions = len(action_map)
        log(f"Action set: reduced {action_map} (NOOP, RIGHT, LEFT)")
    ...
    while step < args.decisions:
        ...
        # Initial features for s0 with null context
        ...
        # Prime v_old = Q(s0, a0) without changing weights
        ...
        while not done and step < args.decisions:
            ...
            # 2. Next state features use current action/reward context
            ...
            # 4. Learn using new state/action (v_old already holds Q(s_t, a_t))
            ...
            # Visualization (use threshold to avoid skipping when step increments by >1)
            if next_heatmap_at is not None and step >= next_heatmap_at:
                try:
                    import imageio
                    # Get alphas per feature
                    ...
                    # Compute active bin per pixel for this frame
                    frame_resized = cv2.resize(next_frame, (extractor.width, extractor.height), interpolation=cv2.INTER_AREA)
                    binned = frame_resized // 32  # 0-7 bins

                    # Gather alphas for the active bin of each pixel/channel
                    ...
                    # Max over channels to get a single map
